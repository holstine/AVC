...
#define ACCEL_X_MIN ((float) -380)
#define ACCEL_X_MAX ((float) 365)
#define ACCEL_Y_MIN ((float) -390)
#define ACCEL_Y_MAX ((float) 365)
#define ACCEL_Z_MIN ((float) -612)
#define ACCEL_Z_MAX ((float) 403)
...
float MAGN_X_MIN  = -149.80; 
float MAGN_X_MAX  = 374.71;
float MAGN_Y_MIN  = -356.28;
float MAGN_Y_MAX  = 145.9;
float MAGN_Z_MIN  = -500;
float MAGN_Z_MAX  = 500;
...
#define GYRO_AVERAGE_OFFSET_X ((float)19.3)
#define GYRO_AVERAGE_OFFSET_Y ((float) -8.5)
#define GYRO_AVERAGE_OFFSET_Z ((float) 20.25)
...
#define GYRO_GAIN 0.06357 // Same gain on all axes
...
void reset_sensor_fusion() {
	...
	pitch = atan2(temp2[1], temp2[2]);

	averagePitch = (.99 * averagePitch + .01 * pitch);
	averageRoll = (.99 * averageRoll + .01 * roll);
	// GET YAW
	Compass_Heading();
	yaw = MAG_Heading;
...
float getCompassHeading(){
	float MAGNETIC_DECLINATION = -8.5; // compass points to the right of trueNorth
	float trueHeading = TO_DEG(MAG_Heading) + MAGNETIC_DECLINATION;
	if(trueHeading <0) trueHeading+=360;
	return trueHeading;
}

float getCompassRate(){
	return -GYRO_GAIN * gyro[2];
}
...
void calibrate_mag(){
//	magnetom[0] -= (15 / .1) *averagePitch * ROLL_PITCH_FUDGE;
//	magnetom[1] += (10 / .05) *averageRoll * ROLL_PITCH_FUDGE;

	if(magnetom[0] < MAGN_X_MIN) MAGN_X_MIN = magnetom[0];
	if(magnetom[0] > MAGN_X_MAX) MAGN_X_MAX = magnetom[0];

	if(magnetom[1] < MAGN_Y_MIN) MAGN_Y_MIN = magnetom[1];
	if(magnetom[1] > MAGN_Y_MAX) MAGN_Y_MAX = magnetom[1];

	if(magnetom[2] < MAGN_Z_MIN) MAGN_Z_MIN = magnetom[2];
	if(magnetom[2] > MAGN_Z_MAX) MAGN_Z_MAX = magnetom[2];

	Serial<< MAGN_X_MIN <<" "<< MAGN_X_MAX <<" "<< MAGN_Y_MIN <<" "<< MAGN_Y_MAX <<endl;
...
#else
	magnetom[0] = (magnetom[0] - MAGN_X_OFFSET - manual_magn_offset[0]) * MAGN_X_SCALE * manual_magn_scale[0];
	magnetom[1] = (magnetom[1] - MAGN_Y_OFFSET - manual_magn_offset[1]) * MAGN_Y_SCALE * manual_magn_scale[1];
	magnetom[2] = (magnetom[2] - MAGN_Z_OFFSET - manual_magn_offset[2]) * MAGN_Z_SCALE * manual_magn_scale[2];
...
void Compass_Heading()
{
	...
	// Magnetic Heading
//	MAG_Heading = atan2(mag_y, mag_x);// 3d
	MAG_Heading = atan2(magnetom[1], magnetom[0]);
}
/* This file is part of the Razor AHRS Firmware */
